...
export type CalendarProps = React.ComponentProps<typeof DayPicker> & {
  /** Нижняя и верхняя граница года для выпадающего выбора (режим dropdown). */
  fromYear?: number;
  toYear?: number;
};
...
/** Подпись календаря с выпадающими списками месяца и года (для дат рождения). */
function DropdownCaption({
  displayMonth,
  fromYear,
  toYear,
}: CaptionProps & { fromYear: number; toYear: number }) {
    ...
    <div className="flex items-center justify-between gap-2 px-1 pt-1">
      <button
        ...
        className={cn(
          ...
          "size-7 bg-transparent p-0 opacity-60 hover:opacity-100 disabled:opacity-30",
        )}
      >
      ...
      <div className="flex flex-1 items-center gap-1.5">
        ...
        >
          <SelectTrigger className="h-7 flex-1 px-2 text-xs">
            <SelectValue />
          </SelectTrigger>
...
export function Calendar({
  ...
  const useDropdown = fromYear != null && toYear != null;
